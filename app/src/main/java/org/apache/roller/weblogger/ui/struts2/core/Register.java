...
public class Register extends UIAction implements ServletRequestAware {
    ...
    private final MailUtilInstance mailUtilInstance;
    private final WebloggerRuntimeConfigInstance webloggerRuntimeConfigInstance;
    ...
    public Register() {
        this(MailUtilInstance.INSTANCE, WebloggerRuntimeConfigInstance.INSTANCE);
    }

    // Test constructor
    Register(MailUtilInstance mailUtilInstance, WebloggerRuntimeConfigInstance webloggerRuntimeConfigInstance) {
        this.mailUtilInstance = mailUtilInstance;
        this.pageTitle = "newUser.addNewUser";
        this.webloggerRuntimeConfigInstance = webloggerRuntimeConfigInstance;
    }
    ...
    public String execute() {
        ...
        try {
            if (!webloggerRuntimeConfigInstance.getBooleanProperty("users.registration.enabled")
                // unless there are 0 users (need to allow creation of first user)
                && getWeblogger().getUserManager().getUserCount() != 0) {
    ...
    public String save() {
        ...
        if (!hasActionErrors()) {
            try {
                ...
                UserManager mgr = getWeblogger().getUserManager();
                ...
                boolean activationEnabled = webloggerRuntimeConfigInstance.getBooleanProperty("user.account.email.activation");
                ...
                if (activationEnabled && ud.getActivationCode() != null) {
                    try {
                        ...
                        mailUtilInstance.sendUserActivationEmail(ud);
                ...
                servletRequest.getSession().removeAttribute(RollerSession.ROLLER_SESSION);
    ...
    public void myValidate() {
        ...
        if (getAuthMethod().equals(AuthMethod.OPENID.name()) ||
                (getAuthMethod().equals(AuthMethod.DB_OPENID.name()) && !StringUtils.isEmpty(bean.getOpenIdUrl()))) {
    ...
    // for Extract&Override in pindown tests only
